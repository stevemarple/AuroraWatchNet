...
\ifdef{\caluniumMagManual}{%
  ...
  magnetometer and data recording daemon. This section is not used
  ...
  only when an ethernet connection with the magnetometer is in use.
...
\ifdef{\caluniumMagManual}{%
  ...
  string, without any \code{0x} prefix. The key used here must match
...
it defaults to the site abbreivation prefixed with \code{awn-}.
...
\code{remote_key2}. The suffix must not contain whitespace characters.
