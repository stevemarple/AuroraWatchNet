...
public:
  ...
  inline CommsHandler(Stream &s, void* stackBuffer, uint16_t stackBufferLen);
...
CommsHandler::CommsHandler(Stream &s, void* stackBuffer,
			   uint16_t stackBufferLen) :
  xrfPoweringUp(false), xrfResetting(false),
  messageLen(0),
  state(stateWaitingForMessages), xrf(s), stack(stackBuffer, stackBufferLen)
{
  ;
}
