...
void CommsHandler::setup(uint8_t sleepPin, uint8_t onPin, uint8_t resetPin)
{
  ...
  delay(1050);
  comms << "ATRE\r";
  comms << "ATSM 2\r"; // Sleep mode
  uint8_t channelNum = eeprom_read_byte((const uint8_t*)EEPROM_RADIO_CHANNEL);
  if (channelNum != 0xFF) {
    comms << "ATCN " << int(channelNum) << '\r';
    console << "Channel number: " << channelNum << endl;
  }
  comms << "ATAC\r"    // Apply changes
	<< "ATDN\r";   // Done
  delay(300);
  while (comms.available()) {
    comms.read(); //Serial.print(comms.read());
  }
...
/*
void CommsHandler::begin(CommsInterface& ci)
{
  comms = ci;
}
*/
