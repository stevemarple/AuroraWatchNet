...
class EEPROM(object):
    '''AuroraWatch EEPROM memory layout'''
    @staticmethod
    def parse_unpack_format(fmt):
        '''Parse the struct pack format string.

        fmt must be a struck.pack format string in the style "!10c" (ie 
        byte order, quantity, data type).
        
        Returns array: order, quantity, type. Quantity is not the same as
...
class Message(object):
    ...
    timestamp_fraction_offset = (timestamp_seconds_offset + 
...
#        pass
    ...
    def format_tag_array_of_longs(tag_name, dataLen, payload):
        return repr(list(struct.unpack('!' + str(dataLen/4) + 'l', 
                                       str(payload))))
    ...
    # have variable length are are entered as zero bytes for length;
    # they are sent as tag, payload length (16-bit unsigned int),
    # payload.
    #
...
class Sensor(object):
    ...
    def __repr__(self):
        return 'config_file: ' + repr(self.config_file)
